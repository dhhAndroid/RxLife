 ...
 * 自动绑定生命周期,请查看：[LifecycleTransformer.convertLifecycle]生命周期转化函数。
 */
fun Completable.bindToLifecycle(owner: LifecycleOwner) = compose(RxLife.with(owner).bindToLifecycle<Any>())

/**
 * 指定在[event]生命周期事件注销
 */
fun Completable.bindUntilEvent(owner: LifecycleOwner, event: Lifecycle.Event) = compose(RxLife.with(owner).bindUntilEvent<Any>(event))

/**
 * 在[Lifecycle.Event.ON_DESTROY],即界面销毁的时候注销
 */
fun Completable.bindOnDestroy(owner: LifecycleOwner) = compose(RxLife.with(owner).bindOnDestroy<Any>())
